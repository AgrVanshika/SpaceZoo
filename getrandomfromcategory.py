...
def getRandomFromCategory(category):
  ...
  with conn.cursor() as cur:
    ...
    #prints in format (name,description,source)
     print(f"('{technique[0]}','{technique[1]}','{technique[2]}')")
     
def main():
  #takes the cockroach connection string and formats it with user and password
  conn_string = "postgresql://{}:{}@free-tier.gcp-us-central1.cockroachlabs.cloud:26257/defaultdb?sslmode=verify-full&sslrootcert=$HOME/.postgresql/root.crt&options=--cluster%3Dzany-yak-4847".format(input("User: "), getpass())
  global conn
  conn = psycopg2.connect(os.path.expandvars(conn_string))
  getRandomFromCategory(argv[1])
